...
exports.edit = (req, res, nxt) => {
    ...
    Item.findOnedAndUpdate({_id: req.params.id, id_categoria: "62d017a1181c3910ccfd43d1"}, {$set: req.body}, {new: true})
    .select('-_id -__v')
    .populate({path : 'id_status' , select: '-_id'})
...
exports.delete = (req, res, nxt) => {
    Item.findOneAndUpdate({_id: req.params.id, id_categoria: "62d017a1181c3910ccfd43d1"}, {id_status: mongoose.Types.ObjectId("62cec7b263187bb9b498687e")}, {new: true})
    .select('-_id -__v')
    .populate({path : 'id_status' , select: '-_id'})
    .then((doc) => (res.status(200).json(doc)))
    .catch((err) => (res.status(500).json(err)));
